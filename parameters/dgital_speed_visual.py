# more enchanced digital speed indicaters:
import cv2
import numpy as np

def draw_speed_meter(frame, center, speed, max_speed, label):
    """ Draws a circular speed meter with a needle and a digital speed display. """
    radius = 50
    angle_range = 270  # Needle moves within a 270° arc (-135° to +135°)
    min_angle = -135
    max_angle = 135
    ...
    # Add digital speed display below the meter
...
while cap.isOpened():
    ...
    drone_speed = np.random.uniform(0, 50)  # Simulated drone speed (0-50 m/s)
    wind_speed = np.random.uniform(0, 30)   # Simulated wind speed (0-30 m/s)
